...
> Since it is quite sensitive and reoccuring topic in the DIY-keyboards community I would not touch it here, but this option is not preferrable for me personally. 
...
- Low-ish chance of connecting it to the wrong device accidentally, since finding spare USB-A to USB-A cable on your desk, connected to some of your devices is highly unlikely (unlike USB-c, for example). Not to mention, using the same port type to connect keyboard to PC and left MCU to the right MCU can be somewhat confusing, and could easily result in  some MCU damage, being connected improperly).
